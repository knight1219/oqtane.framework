...
{
    public class SettingService : HttpService<Setting>, ISettingService
    {
        private readonly HttpClient http;
        private readonly SiteState sitestate;
        private readonly NavigationManager NavigationManager;

        public SettingService(HttpClient http, SiteState sitestate, NavigationManager NavigationManager) : base(http, sitestate, NavigationManager)
        {
            this.http = http;
        ...
        public async Task<Dictionary<string, string>> GetSettingsAsync(string EntityName, int EntityId)
        {
            ...
            List<Setting> Settings = await http.GetJsonAsync<List<Setting>>(this.ApiUrl + "?entityname=" + EntityName + "&entityid=" + EntityId.ToString());
        ...
        {
            return await http.GetJsonAsync<Setting>(this.ApiUrl + "/" + SettingId.ToString());
        }

        public async Task<Setting> AddSettingAsync(Setting Setting)
        {
            return await http.PostJsonAsync<Setting>(this.ApiUrl, Setting);
        }

        public async Task<Setting> UpdateSettingAsync(Setting Setting)
        {
            return await http.PutJsonAsync<Setting>(this.ApiUrl + "/" + Setting.SettingId.ToString(), Setting);
        }

        public async Task DeleteSettingAsync(int SettingId)
        {
            await http.DeleteAsync(this.ApiUrl + "/" + SettingId.ToString());
        }
